...
clip_box = gpd.GeoDataFrame(
    geometry=[box(bounds["minx"], bounds["miny"],
...
FROM census.gadoe2024_389 g
JOIN "allhsgrades24"."tbl_approvedschools" s
  ON g."UNIQUESCHOOLID" = s."UNIQUESCHOOLID"
WHERE s.lat IS NOT NULL AND s.lon IS NOT NULL
"""
school_gdf = gpd.read_postgis(school_query, engine, geom_col='geom')
school_gdf['black_disparity'] = pd.to_numeric(
    school_gdf['RI_Black'], errors='coerce')

# clip to Atlanta bounding box
school_gdf = gpd.clip(school_gdf, clip_box)

# assign color to each school based on disparity bin
disparity_bins = [-0.864929, -0.168258, -0.088616, -
                  0.050715, 0.049845, 0.088608, 0.139144, 0.684211]
disparity_colors = ['#7f2704', '#d94801', '#fdae6b',
                    '#ffffff', '#9ecae1', '#3182bd', '#08519c']
...
school_gdf['disparity_color'] = school_gdf['black_disparity'].apply(
    classify_disparity)
...
pop_df = pd.read_sql(pop_sql, engine)
pop_df['geoid'] = pop_df['geoid'].astype(str).str.zfill(12)
pop_df['black_ratio'] = pop_df['black_alone_non_hispanic'] / \
    pop_df['total_population']

# load block group geometries
block_query = 'SELECT "GEOID", ST_Transform(cbgpolygeom, 4326) AS geom FROM "allhsgrades24"."tbl_cbg_finalassignment"'
...
fig.savefig("output_394/map8_county_394.png", dpi=300,
            bbox_inches='tight', bbox_extra_artists=[leg1, leg2, leg3])
#plt.show()
