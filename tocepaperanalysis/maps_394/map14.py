...
# load high school data and logic classifications using SQLAlchemy engine
school_query = """
SELECT "UNIQUESCHOOLID", lat, lon
FROM "allhsgrades24"."tbl_approvedschools"
WHERE "GRADE_RANGE" LIKE '%9-%'
   OR "GRADE_RANGE" LIKE '10-%'
   OR "GRADE_RANGE" LIKE '11-%'
   OR "GRADE_RANGE" LIKE '12-%'
"""
school_df = pd.read_sql(school_query, engine)

logic_query = 'SELECT "UNIQUESCHOOLID", "LOGIC_CLASS" FROM census.gadoe2024_389'
logic_df = pd.read_sql(logic_query, engine)
...
fig.savefig("output_394/map14_county_366.png", dpi=300,
